...
`github:mat3ra/workflow-designer#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/workflow-designer/releases/download/wip-<short-sha>/workflow-designer.tgz
```
...
```bash
npm run mat3ra:install -- workflow-designer wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/workflow-designer@<release-url>` with
