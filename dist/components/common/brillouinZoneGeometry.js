 ...
 */
const PRIMITIVE_VECTORS_BY_LATTICE_TYPE = {
    CUB: [
        [1, 0, 0],
        [0, 1, 0],
        [0, 0, 1],
 ...
 */
function reciprocalVectors(a1, a2, a3) {
 ...
 */
export function computeBrillouinZoneFacesFromReciprocalVectors(vectors) {
    const [b1, b2, b3] = vectors;
    if ([b1, b2, b3].some((vector) => !vector || vector.length !== 3 || vector.some(Number.isNaN))) {
        return null;
    }
    const reciprocalLatticePoints = [];
    for (let h = -2; h <= 2; h += 1) {
        for (let k = -2; k <= 2; k += 1) {
            for (let l = -2; l <= 2; l += 1) {
                if (h !== 0 || k !== 0 || l !== 0) {
    ...
    const isInsideCell = (point) => planes.every((plane) => dot(point, plane.normal) <= plane.offset + TOLERANCE);
    const vertices = [];
    for (let i = 0; i < planes.length; i += 1) {
        for (let j = i + 1; j < planes.length; j += 1) {
            for (let k = j + 1; k < planes.length; k += 1) {
                const point = solve3x3([planes[i].normal, planes[j].normal, planes[k].normal], [planes[i].offset, planes[j].offset, planes[k].offset]);
                const isCellVertex = Boolean(point) && isInsideCell(point);
                const isDuplicate = isCellVertex &&
                    vertices.some((existing) => length(subtract(existing, point)) < 1e-6);
    ...
    planes.forEach((plane) => {
        const onPlane = vertices.filter((vertex) => Math.abs(dot(vertex, plane.normal) - plane.offset) < 1e-6);
        if (onPlane.length < 3)
            return;
        // Order the polygon by angle around the face normal, in an in-plane basis.
        const normal = normalize(plane.normal);
        const centroid = scale(onPlane.reduce((sum, v) => [sum[0] + v[0], sum[1] + v[1], sum[2] + v[2]], [0, 0, 0]), 1 / onPlane.length);
        ...
        const ordered = [...onPlane].sort((left, right) => {
            ...
            return (Math.atan2(dot(leftOffset, bitangent), dot(leftOffset, reference)) -
 ...
 */
export function computeBrillouinZoneFaces(latticeType) {
