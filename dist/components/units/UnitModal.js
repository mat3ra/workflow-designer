...
export default function UnitModal({ id = "", title = "", className, onClose, unit, units = [], onUpdate, adjustable, editable, isStandalone, onOutputUpdateRequest, materials, materialsIndex, onMaterialSwitch, publicAccount, jobProperties, }) {
    ...
    const renderHeaderCustom = () => {
        return (_jsxs(_Fragment, { children: [_jsx(DialogTitle, { component: "div", children: _jsxs(Grid, { container: true, children: [_jsxs(Grid, { item: true, container: true, justifyContent: "space-between", children: [_jsx(Grid, { item: true, children: _jsx(Typography, { variant: "h5", children: `${unit.name} — ${unit.type} unit` }) }), _jsx(Grid, { item: true, children: _jsx(IconButton, { id: `${id}-close-button`, onClick: onClose, children: _jsx(IconByName, { name: "actions.close", fontSize: "small" }) }) })] }), _jsx(Grid, { item: true, xs: 12, children: _jsx(EntityNameComponent, { editable: editable, value: unit.name, subtitle: { type: unit.type }, onUpdate: (name) => onNameUpdate(name), icon: ENTITY_ICONS.unit, status: getUnitStatusCls(unit.status) }) })] }) }), _jsx(Divider, {})] }));
    };
