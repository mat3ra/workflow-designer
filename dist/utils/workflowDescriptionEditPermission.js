 ...
 */
export function computeIsWorkflowDescriptionEditable(workflowEntity, profile, editable) {
    const currentAccountTeams = Object.values(profile.account.systemTeams);
    const hasPermission = currentAccountTeams.some((team) => {
        return (team.containsEntity(workflowEntity) &&
            team.hasAccessOfTypes(EDIT_ACCESS_TYPE));
    });
